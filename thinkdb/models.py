...
class MySQL_Replication(db.Model):
    ...
    relay_log_file = db.Column(db.String(32),nullable=False,default='',server_default='',comment="RELAY日志")
    relay_log_pos = db.Column(db.String(32), nullable=False, default='',server_default='', comment="RELAY日志偏移量")
    master_log_file = db.Column(db.String(32), nullable=False, default='',server_default='', comment="Master二进制文件名")
    master_log_pos = db.Column(db.String(32), nullable=False, default='',server_default='', comment="Master二进制日志偏移量")
    add_time = db.Column(db.DateTime,default=datetime.now,comment="添加时间")
    last_modify_time = db.Column(db.DateTime,default=datetime.now,onupdate=datetime.now,comment="最后更新时间")
    def __repr__(self):
...
class MySQL_Replication_History(db.Model):
    ...
    db_name = db.Column(db.String(32),nullable=False,index=True,unique=True,comment="名称(标识)")
